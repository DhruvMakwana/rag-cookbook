...
def rewrite_query(query: str, generate_fn) -> str:
    return generate_fn(REWRITE_PROMPT.format(query=query)).strip()


def rewrite_retrieve(question: str, chunks: list[str], embed_model: SentenceTransformer, generate_fn, k: int = 3) -> list[str]:
    import numpy as np

    def cos(a, b):
        return float(np.dot(a, b) / (np.linalg.norm(a) * np.linalg.norm(b) + 1e-8))

    rewritten = rewrite_query(question, generate_fn)
...
def multi_query_retrieve(question: str, chunks: list[str], embed_model: SentenceTransformer, generate_fn, k: int = 3) -> list[str]:
    def cos(a, b):
        return float(np.dot(a, b) / (np.linalg.norm(a) * np.linalg.norm(b) + 1e-8))

    response = generate_fn(MULTI_QUERY_PROMPT.format(query=question))
...
def hyde_retrieve(question: str, chunks: list[str], embed_model: SentenceTransformer, generate_fn, k: int = 3) -> list[str]:
    hypothetical_answer = generate_fn(HYDE_PROMPT.format(query=question))
...
def step_back_retrieve(question: str, chunks: list[str], embed_model: SentenceTransformer, generate_fn, k: int = 3) -> list[str]:
    general_question = generate_fn(STEP_BACK_PROMPT.format(query=question)).strip()
...
def decompose_retrieve(question: str, chunks: list[str], embed_model: SentenceTransformer, generate_fn, k: int = 6) -> list[str]:
    ...
    response = generate_fn(DECOMPOSE_PROMPT.format(query=question))
...
def ircot_retrieve(question: str, chunks: list[str], embed_model: SentenceTransformer, generate_fn, max_hops: int = 3) -> dict:
    ...
    for _ in range(max_hops):
        ...
        step = generate_fn(IRCOT_REASON_PROMPT.format(question=question, context=context_text)).strip()
        reasoning_trace.append(step)

        if step.upper().startswith("ANSWER:"):
            break

        for chunk in naive(step, k=2):
    ...
    return {"reasoning_trace": reasoning_trace, "retrieved_chunks": context_chunks, "final_answer": final_answer}
# --8<-- [end:ircot]
